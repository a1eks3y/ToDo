...
interface registerReq extends express.Request {
    body: {
        email: string,
        password: string,
        username: string,
...
interface loginReq extends express.Request {
    body: {
        email: string,
        password: string
    }
    ...
    async (req: registerReq, res) => {
        try {
            ...
            await transporter.sendMail({
                ...
                text: `Click on this link to confirm email ${config.get('baseUrl')}/api/confirmEmail/${ConfirmEmail}`,
                html: `<div>Click on this link to confirm email <b><a href="${config.get('baseUrl')}/api/confirmEmail/${ConfirmEmail}">${config.get('baseUrl')}/api/confirmEmail/${ConfirmEmail}</a></b></div>`
            })
            ...
            return res.status(201).json({token, userId: user.id, Timezone, message: 'User created'})
        } catch (e) {
        ...
        try {
            ...
            if (typeof user.ConfirmEmail === 'string') {
                await transporter.sendMail({
                    ...
                    to: user.email,
                    subject: 'Confirm email',
                    text: `Click on this link to confirm email ${config.get('baseUrl')}/api/confirmEmail/${user.ConfirmEmail}`,
                    html: `<div>Click on this link to confirm email <b><a href="${config.get('baseUrl')}/api/confirmEmail/${user.ConfirmEmail}">${config.get('baseUrl')}/api/confirmEmail/${user.ConfirmEmail}</a></b></div>`
                })
                return res.status(409).json({message: 'You must confirm email. If you can\'t found letter then check the Spam folder...'})
            }
            const token = jwt.sign(
                {userId: user?.id},
                jwtToken,
